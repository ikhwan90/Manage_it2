...
import manage_it.com.interfaces.dataprovider.BarLineScatterCandleBubbleDataProvider;
import manage_it.com.interfaces.datasets.IDataSet;
import manage_it.com.utils.MPPointD;
 ...
 */
public class ChartHighlighter<T extends BarLineScatterCandleBubbleDataProvider> implements manage_it.com.highlight.IHighlighter
{
     ...
     */
    protected List<manage_it.com.highlight.Highlight> mHighlightBuffer = new ArrayList<manage_it.com.highlight.Highlight>();
    ...
    @Override
    public manage_it.com.highlight.Highlight getHighlight(float x, float y) {

        MPPointD pos = getValsForTouch(x, y);
        float xVal = (float) pos.x;
        MPPointD.recycleInstance(pos);

        manage_it.com.highlight.Highlight high = getHighlightForX(xVal, x, y);
        return high;
    }
     ...
     * @return
     */
    protected manage_it.com.highlight.Highlight getHighlightForX(float xVal, float x, float y) {

        List<manage_it.com.highlight.Highlight> closestValues = getHighlightsAtXValue(xVal, x, y);
        ...
        manage_it.com.highlight.Highlight detail = getClosestHighlightByPixel(closestValues, x, y, axis, mChart.getMaxHighlightDistance());
     ...
     * @return
     */
    protected float getMinimumDistance(List<manage_it.com.highlight.Highlight> closestValues, float pos, YAxis.AxisDependency axis) {

        float distance = Float.MAX_VALUE;

        for (int i = 0; i < closestValues.size(); i++) {

            manage_it.com.highlight.Highlight high = closestValues.get(i);
    ...
    protected float getHighlightPos(manage_it.com.highlight.Highlight h) {
        return h.getYPx();
    }
     ...
     * @return
     */
    protected List<manage_it.com.highlight.Highlight> buildHighlights(IDataSet set, int dataSetIndex, float xVal, DataSet.Rounding rounding) {

        ArrayList<manage_it.com.highlight.Highlight> highlights = new ArrayList<>();
        ...
        for (Entry e : entries) {
            ...
            highlights.add(new manage_it.com.highlight.Highlight(
                    e.getX(), e.getY(),
     ...
     * @return
     */
    public manage_it.com.highlight.Highlight getClosestHighlightByPixel(List<manage_it.com.highlight.Highlight> closestValues, float x, float y,
                                                                                       YAxis.AxisDependency axis, float minSelectionDistance) {

        manage_it.com.highlight.Highlight closest = null;
