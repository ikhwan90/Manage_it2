...
public class TimerService extends Service {
    ...
    private Handler mHandler = new Handler(){
        ...
        public void handleMessage(Message msg) {
            if(msg.what == MESSAGE_TIMER_UPDATE){
                ...
                if(remainingTime > 0){
                    BusProvider.getInstance().post(new CountDown(remainingTime));
    ...
    public void onDestroy() {
        ...
        if(StartTimer.timerMode == TimerMode.WORK && status != TimerStatus.STOPPED){
    ...
    @Subscribe
    public void onStartCommand(StartTimer event){
        if (TimerProperties.getInstance().getTimerStatus() == TimerStatus.PAUSED){
            resumeTimer();
        }else{
            startTimer(StartTimer.timerMode);
        }
    }

    @Subscribe
    public void onPauseCommand(PauseTimer event){
        pauseTimer();
    }

    @Subscribe
    public void onStopCommand(StopTimer event){
        stopTimer();
        if (StartTimer.timerMode == TimerMode.WORK){
    ...
    @Subscribe
    public void onForegroundCommand(StartForeground event){
        if(StartForeground.startForeground){
