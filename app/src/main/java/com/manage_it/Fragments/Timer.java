...
import com.manage_it.Utils.AppNotifications;
import com.manage_it.Utils.AppPreferences;
...
import com.manage_it.Events.CountDown;
import com.manage_it.Events.PauseTimer;
import com.manage_it.Events.StartTimer;
import com.manage_it.Events.StopTimer;
import com.manage_it.Activities.Settings;
import com.manage_it.ToDo;
import manage_it.com.circularprogressbar.HoloCircularProgressBar;
...
import static com.manage_it.TimerMode.BREAK;
import static com.manage_it.TimerMode.LONG_BREAK;
import static com.manage_it.TimerMode.WORK;


public class Timer extends Fragment implements
...
//        PreferenceManager.setDefaultValues(getActivity(), R.xml.app_prefs, false);
    ...
    public void onStop() {
        ...
        if(mTimerMode == WORK && mIsWorkActive && FocusTaskChanged.currentFocusTask.getToDoId()!=99999){
            ...
            if(FocusTaskChanged.currentFocusTask.getToDoId() == 99999){
                ...
                if(!mIsWorkActive){
                    BusProvider.getInstance()
                            .post(new FocusTaskChanged(new ToDo(99999,"Free task")));
    ...
    private void setButtonListeners() {
        ...
        mButtonResume.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                BusProvider.getInstance().post(new StartTimer(mTimerMode));
                mButtonResume.setVisibility(View.INVISIBLE);
                mButtonResume.setEnabled(false);
                mButtonStop.setVisibility(View.INVISIBLE);
                mButtonStop.setEnabled(false);
    ...
    private void startTimer(TimerMode timerMode) {
        ...
        switch (timerMode){
            case WORK:
                mIsWorkActive = true;
                timerDuration = appPreferences.getWorkDuration();
                BusProvider.getInstance().post(new StartTimer(timerMode));
    ...
    private void pauseSkip() {
        if(mTimerMode == WORK){
            BusProvider.getInstance().post(new PauseTimer());
            mButtonPause.setVisibility(View.INVISIBLE);
            mButtonPause.setEnabled(false);
            ...
            BusProvider.getInstance().post(new StopTimer());
            mTimerMode = WORK;
            view.setBackgroundColor(ContextCompat.getColor(getActivity(),R.color.colorPrimary));
            mCircularProgressBar.setProgressBackgroundColor(ContextCompat.getColor(getActivity(),R.color.colorPrimaryDark));
            listener.onTimerModeChanged(WORK);
    ...
    @Subscribe
    public void onCountDown(CountDown event){
        updateTimerLabel(CountDown.remainingTime);
    }
