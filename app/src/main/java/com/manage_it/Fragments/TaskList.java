...
import com.manage_it.Activities.MainActivity;
import com.manage_it.Adapters.ToDoList;
import com.manage_it.Events.FocusTaskChanged;
import com.manage_it.Activities.About;
import com.manage_it.Activities.CompletedTask;
import com.manage_it.Activities.Settings;
import com.manage_it.Activities.Task;
import com.manage_it.Events.CurrentTaskEdited;
import com.manage_it.Events.CurrentTaskChecked;
import com.manage_it.Events.StartTimer;
...
import com.manage_it.Utils.DBHelper;
...
import static com.manage_it.Activities.Task.CHANGES_MADE;
import static com.manage_it.Activities.Task.TASK_DUE_DATE;
import static com.manage_it.Activities.Task.TASK_NAME;
import static com.manage_it.Activities.Task.TASK_NOTE;
import static com.manage_it.Utils.DBHelper.COLUMN_DUE_DATE;
import static com.manage_it.Utils.DBHelper.COLUMN_NAME;
import static com.manage_it.Utils.DBHelper.COLUMN_NOTE;
import static com.manage_it.Utils.DBHelper.COLUMN_STATUS;
import static com.manage_it.Utils.DBHelper.DML_INSERT;
import static com.manage_it.Utils.DBHelper.DML_TYPE;
import static com.manage_it.Utils.DBHelper.DML_UPDATE;
import static com.manage_it.Utils.DBHelper.INCOMPLETE;
...
public class TaskList extends Fragment {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mAdapter = new ToDoList(mToDoList,getActivity());
    ...
    public void onStop() {
        ...
        if(mCurrentFocusTaskPosition != NOT_DEFINED &&
                StartTimer.timerMode == TimerMode.WORK &&
        ...
        if (resultCode == RESULT_OK){
            if(requestCode == REQUEST_CODE_COMPLETED){
                ...
                if(data.getBooleanExtra(CHANGES_MADE,false)){
                    ...
                    ToDo toBeEditedItem = mToDoList.get(toBeEditedPosition);
                    ...
                    if(toBeEditedPosition == mCurrentFocusTaskPosition){
                        BusProvider.getInstance().post(new CurrentTaskEdited(toBeEditedItem));
                    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.action_view_completed) {
            ...
            Intent completedTasksIntent = new Intent(getActivity(), CompletedTask.class);
 ...
 /*       }else if (id == R.id.action_about){
    ...
    private void setClickListeners() {
        ...
        mFab.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v){
                ...
                Intent taskIntent = new Intent(getActivity(),Task.class);
                taskIntent.putExtra(DML_TYPE, DML_INSERT);
                startActivityForResult(taskIntent,REQUEST_CODE_INSERT);
            }
        });

        mAdapter.setItemMenuListener(new ToDoList.ItemMenu() {
            //edit task
            @Override
            public void editTask(int position) {
                if(position == mCurrentFocusTaskPosition &&
                    ...
                    ToDo item = mToDoList.get(toBeEditedPosition);
                    Intent taskIntent = new Intent(getActivity(),Task.class);
                ...
                if(taskPosition == mCurrentFocusTaskPosition &&
                    ...
                    deleteDialogOk.setPositiveButton(R.string.delete, new DialogInterface.OnClickListener() {
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            ...
                            if(taskPosition == mCurrentFocusTaskPosition){
                                mCurrentFocusTaskPosition = NOT_DEFINED;
                                ToDo item = new ToDo(99999,"Free task");
                                BusProvider.getInstance().post(new FocusTaskChanged(item));
            ...
            public void markAsComplete(int position) {
                if(mCurrentFocusTaskPosition == position){
                    ...
                    BusProvider.getInstance().post(new CurrentTaskChecked());
            ...
            public void setFocusTask(int position) {
                if(mCurrentFocusTaskPosition == position){
                    ...
                    if(TimerProperties.getInstance().getTimerStatus() == TimerStatus.STOPPED){
                        ...
                        BusProvider.getInstance().post(new FocusTaskChanged(mToDoList.get(position)));
    ...
    private void onWorkSessionFinished(int secondsWorked) {
        ...
        if(mCurrentTaskChecked){
            ...
            item = FocusTaskChanged.previousFocusTask;
    ...
    private class LoadTasks extends AsyncTask<Object,Void,List<ToDo>> {

        @Override
        protected List<ToDo> doInBackground(Object... params) {
            return mDbHelper.getAllTasks(INCOMPLETE);
        }

        @Override
        protected void onPostExecute(List<ToDo> toDoItemList) {
            if(toDoItemList.isEmpty()){
                ...
                if(mTasksUnTicked){
                    ...
                    long id = FocusTaskChanged.currentFocusTask.getToDoId();
